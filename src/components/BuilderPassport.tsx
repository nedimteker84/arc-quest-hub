...
function BuilderPassport({
  ...
  async function createIpfsTokenUri() {
    const response = await fetch("/api/passport-metadata", {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        ...
        xp: passport.xp,
        builderScore: passport.builderScore,
        reputation: passport.reputation,
        currentStreak: passport.currentStreak,
        bestStreak: passport.bestStreak,
        totalCheckIns: passport.totalCheckIns,
        badges: passport.badges,
        externalUrl: passportUrl,
      }),
    })
    ...
    if (!response.ok || !data.tokenUri) {
      throw new Error(data.error ?? "Failed to create IPFS metadata.")
    }
  ...
  async function mintPassportNft() {
    ...
    try {
      const tokenUri = await createIpfsTokenUri()
      ...
      alert(error instanceof Error ? error.message : "Passport NFT mint failed.")
    } finally {
      setMinting(false)
    }
  }
